...
async function getDataBySubCategory(subid,page=1,color,brand,size,minPrice,maxPrice,showPrices) {
    const res=await axios.get(`https://ecommerce.ibradev.me/products/all?subcategoryId=${subid}&page=${page}${color?.length>0 ? `&color=${color?.map(item=> item).join('%2C')}` : ''}${size?.length ? `&size=${size.map(item => item).join(',')}` : ''}${brand ? `&brandId=${brand}` : ''}${showPrices && minPrice!=0  ? `&minPrice=${minPrice}` : ''}${showPrices && maxPrice!=4500  ? `&maxPrice=${maxPrice}` : ''} `)
    return res.data
}
